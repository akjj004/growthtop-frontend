...
const Products = ({
  products: { name, position, price, rating, thumbnail, numberOfOffers, url, refreshed_at, credibility },
}) => {
  return (
    <Link href={`${url}/${position}`} passHref>
      <Flex flexWrap="wrap" w="420px" p="5" paddingTop="0px" justifyContent="flex-start" cursor="pointer">
        ...
        <Box w="full">
          ...
          <Text>Credibility: {credibility}</Text>

          <Text fontSize="lg">{name.length > 30 ? name.substring(0, 30) + '...' : name}</Text>
        </Box>
      </Flex>
    </Link>
  );
